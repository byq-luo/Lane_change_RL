...
if not options.duaonly:
    ...
    execute("sumo -W --no-step-tf_events -n %s -e 90000 -r %s/routes.rou.xml --dump-basename dump_successive " +
            "--dump-intervals 900 --emissions emissions_successive.xml --tripinfo-output tripinfo_successive.xml " +
            "%s -l sumo_successive.tf_events" % (netFile, succDir, sumoAdds))
    execute("sumo -W --no-step-tf_events -n %s -e 90000 -r %s/routes.rou.xml --dump-basename dump_clogit " +
            "--dump-intervals 900 --emissions emissions_clogit.xml --tripinfo-output tripinfo_clogit.xml " +
            "%s -l sumo_clogit.tf_events" % (netFile, clogDir, sumoAdds))
    execute("sumo -W --no-step-tf_events -n %s -e 90000 -r %s/routes.rou.xml --dump-basename dump_lohse " +
            "--dump-intervals 900 --emissions emissions_lohse.xml --tripinfo-output tripinfo_lohse.xml %s -l " +
            "sumo_lohse.tf_events" % (netFile, lohseDir, sumoAdds))
    tripinfos += tripinfofile + \
